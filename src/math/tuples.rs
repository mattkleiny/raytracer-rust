...
impl Tuple {
  pub fn is_vector(&self) -> bool { self.w.is_approx(0.) }
  pub fn is_point(&self) -> bool { self.w.is_approx(1.) }
...
mod tests {
  ...
  fn tuples_should_have_unit_length_normalized_vector() {
    let normal = vec(1., 2., 3.).normalize();

    assert!(normal.magnitude().is_approx(1.));
  }
}