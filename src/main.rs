...
impl Scene {
  ...
  fn trace(&self, ray: Ray, depth: usize, max_depth: usize) -> Color {
    ...
    if let Some(intersection) = self.find_intersecting_object(ray) {
  ...
  fn find_intersecting_object(&self, ray: Ray) -> Option<(&Box<dyn SceneNode>, f64)> {
    let mut distance = 999999999.0;
    let mut result: Option<(&Box<dyn SceneNode>, f64)> = None;
